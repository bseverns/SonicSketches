...
void draw()
{
  ...
  text("Left click to start/stop recording. Right click to play.", 100, 100); // tell the user what to do!
...
void mousePressed()
{
  ...
  {
    SamplePlayer sp = new SamplePlayer(ac, targetSample); // instantiate a new SamplePlayer with the recorded sample
    g.addInput(sp); // connect the SamplePlayer to the Gain
    sp.setKillOnEnd(true); // tell the SamplePlayer to destroy itself when it finishes
    sp.start(); // and begin playback
  }
}
